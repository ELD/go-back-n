...
simulator::simulator()
{
    ...
    kr_time = 0.0;           /* initialize time to 0.0 */
...
void simulator::go()
{
    ...
    while (1) {
        ...
        kr_time = eventptr->evtime; /* update time to next event time */
        if (nsim == nsimmax)
...
void simulator::tolayer3(int AorB, struct pkt packet)
{
    ...
    if (TRACE > 2) {
        printf("          TOLAYER3: seq: %d, ack %d, check: %d ", mypktptr->seqnum, mypktptr->acknum,
               mypktptr->checksum);
        for (i = 0; i < 20; i++)
...
void simulator::tolayer5(int AorB, char datasent[20])
{
    int i;
    if (TRACE > 2) {
